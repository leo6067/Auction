...
public class AutoFlowLayout<T> extends ViewGroup {
    ...
    /**
     * 单选的时候 是否允许点击同一个item 取消选中的功能
     */
    private boolean mIsCancleSelect;
    /**
     * 被选中的pos
     */
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mIsCancleSelect = ta.getBoolean(R.styleable.AutoFlowLayout_isCancleSelect, false);
        if (mColumnNumbers != 0) {
            mIsGridMode = true;
        }
        ta.recycle();
    ...
    private void setGridMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (tempWidth > sizeWidth) {
            widthResult = sizeWidth / mColumnNumbers;
        } else {
            widthResult = tempWidth / mColumnNumbers;
        }
        //宽高超过屏幕大小，则进行压缩存放
...
//        }
    ...
    private void setChildClickOperation(View child, final Integer key) {
        ...
        child.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                mSelectPos=(Integer) (key == -1 ? view.getTag() : key);
                if (mOnItemClickListener != null) {
                    mOnItemClickListener.onItemClick(mSelectPos, view);
                }
            }
        });
    }
     ...
     * @return 废弃
     */
    public boolean deleteView2(int index) {
     ...
     *
     * @param adapter
     * @param defaultSelectPos 是否默认选中某一个item
     */
    public void setAdapter(FlowAdapter<T> adapter,int defaultSelectPos) {
        clearViews();
        mAdapter = adapter;
    ...
    //grid布局重新计算控件的宽度 使其能够平分屏幕宽度
    public void autoGridSize(Context context, View view) {
        Resources resources = context.getResources();
        DisplayMetrics dm = resources.getDisplayMetrics();
        int width = (int) (dm.widthPixels - (mColumnNumbers - 1) * getHorizontalSpace() - getPaddingLeft() - getPaddingRight());
        LayoutParams tvLp = view.getLayoutParams();
        tvLp.width = (int) (width / mColumnNumbers);
