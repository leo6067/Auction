...
object XYShare : BaseShare() {
    ...
    fun share(activity: Activity, images: Array<Bitmap>, options: Options) {
        Constant_android.stringProhibitRepeatTag = true

        activity.runOnUiThread {
            if (!TextUtils.isEmpty(options.text)) {
                ClipboardUtil.setPrimaryClip(activity, "", options.text)
            }
    ...
    private fun startShare(activity: Activity, fileList: List<File>, options: Options) {
        if (!options.isAutoFill || WXShareMultiImageHelper.isServiceEnabled_XY(activity)) {
        ...
        CustomerDialogUtils.getInstance().showUpdateDialog(activity, true, activity.getString(R.string.xy_share_dialog_title),
                activity.getString(R.string.xy_share_dialog_message),
            ...
            CustomerDialogUtils.getInstance().dissUpdateDialog()
            ServiceManager_XY.openService(activity) {
    ...
    private fun shareToXYUIAuto(context: Context, options: Options, uriList: List<Uri>) {
        ...
        intent.component = ComponentName(XY_PACKAGE_NAME, XY_LAUNCHER_UI)
        intent.type = "image/*"
     ...
     */
     override fun checkShareEnable(context: Context): Boolean {
        ...
        if (!WXDetectUtil.isXYInstalled(context)) {
            Toast.makeText(context, "未安装闲鱼", Toast.LENGTH_SHORT).show()
            return false
        }
        return true
    }
