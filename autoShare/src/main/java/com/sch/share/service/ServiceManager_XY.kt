...
object ServiceManager_XY {
    ...
    fun isServiceEnabled_XY(context: Context): Boolean {
        (context.getSystemService(Context.ACCESSIBILITY_SERVICE) as AccessibilityManager)
                ...
                .filter {
                    ...
                    "${context.packageName}" in it.id && "${XYShareMultiImageService::class.java.simpleName}" in it.id
                }
    ...
    fun openService(context: Context, listener: (Boolean) -> Unit) {
        if (isServiceEnabled_XY(context)) {
            listener(true)
            return
        }
        (context.applicationContext as Application).registerActivityLifecycleCallbacks(object : ActivityLifecycleCallbacksAdapter() {
            override fun onActivityResumed(activity: Activity?) {
                if (context::class.java == activity!!::class.java) {
                    ...
                    listener(isServiceEnabled_XY(context))
                }
            }
        })
        //打开系统无障碍设置界面
     ...
     * @param listener 打开服务结果监听。
     */
    fun openService(context: Context, listener: ServiceManager_XY.OnOpenServiceListener) {
