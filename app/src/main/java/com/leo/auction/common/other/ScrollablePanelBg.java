package com.leo.auction.common.other;
...
import com.leo.auction.utils.ScreenUtils;
...
public class ScrollablePanelBg extends FrameLayout {
    ...
    static PointF mapCenter = new PointF();

    public ScrollablePanelBg(Context context, PanelAdapter panelAdapter) {
        super(context);
        this.panelAdapter = panelAdapter;
        initView(context);
    }

    public ScrollablePanelBg(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView(context);
    }

    public ScrollablePanelBg(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initView(Context mContext) {
        ...
        LayoutInflater.from(getContext()).inflate(R.layout.view_scrollable_panel_bg, this, true);
        ...
        int dw = imageBg.getDrawable().getBounds().width();
        int dh = imageBg.getDrawable().getBounds().height();


        BitmapFactory.Options opts = new BitmapFactory.Options();
        opts.inJustDecodeBounds = true;
        BitmapFactory.decodeResource(getResources(), R.drawable.qq_a, opts);
        opts.inSampleSize = 1;
//        opts.inJustDecodeBounds = false;
        ...
        float x = ScreenUtils.getScreenWidth(mContext) / 2 - width / 2;
        float y = ScreenUtils.getScreenHeight(mContext) / 2 - height / 2;


        Globals.log("log xwj ACTION_MOVE ScreenUtils.getScreenWidth(mContext)   " + ScreenUtils.getScreenWidth(mContext));
        Globals.log("log xwj ACTION_MOVE ScreenUtils.getScreenHeight(mContext)  " + ScreenUtils.getScreenHeight(mContext));
        Globals.log("log xwj ACTION_MOVE imageBg.getWidth() " + width);
        Globals.log("log xwj ACTION_MOVE imageBg.getHeight()  " + height);

        mapCenter.set(x, y);
        Matrix matrix = new Matrix();
        matrix.postTranslate(mapCenter.x, mapCenter.y);
//
        imageBg.setImageMatrix(matrix);
    ...
    /**
     * Adapter used to bind dataSet to cell View that are displayed within every row of {@link com.kelin.scrollablepanel.library.ScrollablePanel}.
     */
    ...
    /**
     * Adapter used to bind dataSet to views that are displayed within a{@link com.kelin.scrollablepanel.library.ScrollablePanel}.
     */
    private static class PanelLineAdapter extends RecyclerView.Adapter<PanelLineAdapter.ViewHolder> {
        ...
        public PanelLineAdapter(PanelAdapter panelAdapter, RecyclerView contentRV, RecyclerView headerRecyclerView) {//左右-上下
            this.panelAdapter = panelAdapter;
            this.headerRecyclerView = headerRecyclerView;
            this.contentRV = contentRV;
            initRecyclerView(headerRecyclerView);
            initConRecycle(contentRV);
        ...
        @Override
        public void onBindViewHolder(PanelLineAdapter.ViewHolder holder, int position) {
        ...
        public void initConRecycle(RecyclerView contentRV) {
            contentRV.addOnScrollListener(new RecyclerView.OnScrollListener() {
                @Override
                public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                    super.onScrolled(recyclerView, dx, dy);
                    Matrix matrix = new Matrix();
                    matrix.postTranslate(mapCenter.x + 0, mapCenter.y - dy);
                    mapCenter.y = mapCenter.y - dy;
                    imageBg.setImageMatrix(matrix);
                }
        ...
        public void initRecyclerView(RecyclerView recyclerView) {
            ...
            recyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
                ...
                public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                    super.onScrolled(recyclerView, dx, dy);
                    Globals.log("log xwj ACTION_MOVE b" + dx + "      " + dy);
                    Matrix matrix = new Matrix();
                    matrix.postTranslate(mapCenter.x - dx, mapCenter.y + 0);
                    mapCenter.x = mapCenter.x - dx;
                    imageBg.setImageMatrix(matrix);
