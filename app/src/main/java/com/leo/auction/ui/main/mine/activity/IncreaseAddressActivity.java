...
public class IncreaseAddressActivity extends BaseActivity {
    ...
    private void sureUpdate() {
        ...
        AddAddressModel.sendAddressRequest(TAG, TextOptionUtils.getInstance().subLength(etConsigneeName.getText().toString().trim(),8),
                ...
                etPostalCode.getText().toString().trim(), sbDefault.isChecked() ? "1" : "0",type,new CustomerJsonCallBack<AddAddressModel>() {
                    @Override
                    public void onRequestError(AddAddressModel returnData, String msg) {
                        hideWaitDialog();
                        showShortToast(msg);
                    }

                    @Override
                    public void onRequestSuccess(AddAddressModel returnData) {
                        hideWaitDialog();
                        showShortToast("添加地址成功");

                        AddressModel.AddressBean item =new AddressModel.AddressBean(provinceId,provinceName,cityId,cityName,areaId,areaName,
