 ...
 */
public class OrderFragment extends BaseRecyclerViewFragment implements SetPaypwdUtils.IComplete, PayPwdBoardUtils.IPayType, WarningDialog.OnWarningClickListener {
...
//    }
    ...
    private int isSeller = 1;// 1 买家 2 卖家
...
//            getWXPayStatus();
    ...
    protected void initAdapter() {
        mAdapter = new OrderAdapter(neetTime, status);
    }
    ...
    public void initEvent() {
        ...
        ((OrderAdapter) mAdapter).setOnItemListsner(mOnItemListsner);
        ((OrderAdapter) mAdapter).setOnBtnListsner(mOnBtnListsner);
    }
...
//                    customerServiceIntervention(item.getRefundId());
    ...
    //同意当面交易
    private void httpFaceJY(int type) {  //1-申请当面交易  2-同意当面交易  4-拒绝当面交易
        showWaitDialog();
        BaseModel.httpFaceTrade(orderCode, type, new HttpRequest.HttpCallback() {
            @Override
            public void httpError(Call call, Exception e) {
                hideWaitDialog();
            }
    ...
    // 延迟付款
    private void httpDelayPay() {
        showWaitDialog();
        BaseModel.httpDelayPay(orderCode, new HttpRequest.HttpCallback() {
            @Override
            public void httpError(Call call, Exception e) {
                hideWaitDialog();
            }
...
//            }
    ...
    public static OrderFragment newIntance(String status) {   //角度  1-买家角度(默认1)   2-卖家角度
