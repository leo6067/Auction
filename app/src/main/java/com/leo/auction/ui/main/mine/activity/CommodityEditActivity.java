...
public class CommodityEditActivity extends BaseActivity implements IReleaseSortChoose, CompressUploadPicUtils.IChoosePic, CompressUploadVideoUtils.IChooseVideo {
    ...
    private ArrayList<ReleaseSortModel.DataBean.AttributesBean> attributes;//商品属性信息
    private ArrayList<String> oldData = new ArrayList<>();//被替换下来得数据
    private UserInfoModel userInfoModel;
    private ProductDetailHeadModel.InfoBean newestReleaseProduct;
    ...
    public void initData() {
        ...
        //初始化图片上传
        uploadPicUtils.initChoosePic(CommodityEditActivity.this, true, 9, Constants.Api.OSS_FOLDER_IMG_GOODS,this);
        //设置拖拽监听
        OnItemDragListener listener = new OnItemDragListener() {
            ...
            public void onItemDragStart(RecyclerView.ViewHolder viewHolder, int pos) {
                ...
                VibratorUtils.getInstance().vibrator(CommodityEditActivity.this, 10);
            }
...
//                holder.setTextColor(R.id.tv, Color.BLACK);
    ...
    private void getProductDetailData() {
        ProductDetailHeadModel.sendProductDetailHeadRequest(TAG, id, new CustomerJsonCallBack<ProductDetailHeadModel>() {
            @Override
            public void onRequestError(ProductDetailHeadModel returnData, String msg) {
                hideWaitDialog();
                showShortToast(msg);
            }

            @Override
            public void onRequestSuccess(ProductDetailHeadModel returnData) {
                hideWaitDialog();
    ...
    //设置页面得初始数据
    private void setPageInitData(ProductDetailHeadModel.InfoBean info) {
        ...
        etStock.setText(EmptyUtils.strEmpty(String.valueOf(info.getStock())));
        ...
        if (info.getVideos() != null && !info.getVideos().isEmpty()) {
            for (String image : info.getVideos()) {
                ReleaseVideoModel releaseVideoModel = new ReleaseVideoModel("2", null, info.getCutPic(), image, "", "");
    ...
    public void initEvent() {
        ...
        //设置销售价格的监听
        textLightUtils.onTextChangedListener(etSellingPrice, "999999999");
        ...
        ReleaseSortModel.sendReleaseSortRequest(TAG, "-1", "", "0", new CustomerJsonCallBack<ReleaseSortModel>() {
            ...
            public void onRequestSuccess(ReleaseSortModel returnData) {
                ...
                if (returnData.getData() != null && !returnData.getData().isEmpty()) {
                    ...
                    if (!EmptyUtils.isEmpty(newestReleaseProduct.getCategoryParentId())) {
                        boolean tag = false;
                        int position = 0;
        ...
        ReleaseSortModel.sendReleaseSortRequest(TAG, releaseOneSortData.getId(), "", "0", new CustomerJsonCallBack<ReleaseSortModel>() {
            ...
            public void onRequestSuccess(ReleaseSortModel returnData) {
                ...
                if (returnData.getData() != null && !returnData.getData().isEmpty()) {
                    ...
                    if (!EmptyUtils.isEmpty(newestReleaseProduct.getCategoryId())) {
                        boolean tag = false;
                        int position = 0;
    ...
    private void setAttributeData(ArrayList<ReleaseSortModel.DataBean.AttributesBean> attributesBeans) {
        ...
        for (ReleaseSortModel.DataBean.AttributesBean attribute : attributes) {
            ...
            if (attribute.getTags() != null && !attribute.getTags().isEmpty()) {
                ...
                if (newestReleaseProduct != null) {
                    for (ProductDetailHeadModel.InfoBean.AttributesBean newestReleaseProductAttribute : newestReleaseProduct.getAttributes()) {
        ...
        public void onClick(View v) {
            if (decryOssDataModel != null) {
                if (zpbProgress.getVisibility() == View.GONE) {
                    ...
                    showShortToast(zpbProgress.getTag() + "正在上传，请稍等");
                }
            } else {
                showShortToast("oss数据请求，稍等会，再试下。还不行，则需要联系客服");
            }
        }
    };
    ...
    private View.OnClickListener mOnImgsItemListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            for (int i = 0; i < postImglistAdapter.getData().size() - 1; i++) {
                imgListsData[i] = "2".equals(postImglistAdapter.getData().get(i).getTag()) ? postImglistAdapter.getData().get(i).getImgPth() : postImglistAdapter.getData().get(i).getFile().getAbsolutePath();
            }
    ...
    private View.OnClickListener mOnImgsItemDeleteListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            if ("0".equals(item.getTag())) {   //5.23  修改，前端不删除图片阿里云图片
                //判断图片是否已上传完成
                if (item.isUploadComplete()) {
                    ...
                    showShortToast("图片还未上传完成");
                }
            } else if ("2".equals(item.getTag())) {
                oldData.add(item.getImgPth());
            ...
            if ("1".equals(item.getTag())) {
                if ("2".equals(item.getUploadCompleteStatus())) {
                    ...
                    ossVideoUtils.initOssOption(CommodityEditActivity.this, decryOssDataModel.getAccessKeyId(), decryOssDataModel.getSecret(), "",
                            decryOssDataModel.getEndPoint(), decryOssDataModel.getBucketName());
                    ossVideoUtils.deleteOssObject(item.getVideoPath(), new OssVideoUtils.OssDeleteListener() {
                        @Override
                        public void deleteSuccess() {
                            hideWaitDialog();
                    ...
                    showShortToast("视频正在处理中");
                }
            } else if ("2".equals(item.getTag())) {
                oldData.add(item.getVideoPath());
    ...
    private void preRelease() {
        if (EmptyUtils.isEmpty(etTitle.getText().toString()) || etTitle.getText().toString().length() < 5 || etTitle.getText().toString().length() > 30) {
            showShortToast("请输入5-30个文字的商品标题");
            return;
        }

        if (EmptyUtils.isEmpty(etContent.getText().toString()) || etContent.getText().toString().length() < 5 || etContent.getText().toString().length() > 500) {
        ...
        if (EmptyUtils.isEmpty(etStock.getText().toString()) || etStock.getText().toString().length() < 1 || etStock.getText().toString().length() > 999) {
            showShortToast("请设置1-999商品库存数量");
            return;
        }
        //判断属性信息是否都按要求填写
        if (attributes != null) {
                ...
                if ("1".equals(attribute.getOption())) {
                    if (attribute.getTags() != null && !attribute.getTags().isEmpty()) {
                        if (((ReleaseAttributeAdapter) crlAttriList.getAdapter()).getmSelectedReleaseSortData() == null) {
                            showShortToast("请选择" + attribute.getTitle() + "属性");
                            return;
                        }
                    } else {
                        if (EmptyUtils.isEmpty(etAttriValue.getText().toString())) {
                            showShortToast("请输入" + attribute.getTitle());
                            return;
                        }
                    }
                }
    ...
    private void release() {
        ...
        CommodityReleaseModel.sendCommodityEditRequest(TAG, id, releaseTwoSortData.getId(), userInfoModel.getShopUri(), etTitle.getText().toString(),
                etContent.getText().toString(), etSellingPrice.getText().toString(), etStock.getText().toString(), etSupplyPrice.getText().toString(),
                sbtFreeShipping.isChecked() ? "1" : "0", sbtToPay.isChecked() ? "1" : "0", sbtFreeShipping.isChecked() ? "0" : etFreight.getText().toString(),
                ossPaths, ossVideoPaths, cutPic, releaseAttributesModels, type, etRemarks.getText().toString().trim(), oldData, new CustomerJsonCallBack<CommodityReleaseModel>() {
                    @Override
                    ...
                    public void onRequestSuccess(CommodityReleaseModel returnData) {
                        hideWaitDialog();
                        if ("1".equals(type)) {
                            showShortToast("商品发布成功");
                            if ("OOB".equals(status)) {
                                BroadCastReceiveUtils.sendLocalBroadCast(CommodityEditActivity.this,
                                        Constants.Action.SEND_REFRESH_COMMODITY_MANAGEMENTFRAGMENT + "00A");
                            }

                        } else {
                            showShortToast("商品修改成功");
                        }

                        BroadCastReceiveUtils.sendLocalBroadCast(CommodityEditActivity.this,
                                Constants.Action.SEND_REFRESH_COMMODITY_MANAGEMENTFRAGMENT + status);

                        BroadCastReceiveUtils.sendLocalBroadCast(CommodityEditActivity.this,
                                Constants.Action.SEND_REFRESH_COMMODITY_MANAGEMENTFRAGMENT);
                        //刷新商品详情页面
                        BroadCastReceiveUtils.sendLocalBroadCast(CommodityEditActivity.this,
                                Constants.Action.SEND_REFRESH_PRODUCT_DETAIL);
                        goFinish();
                    }
                });
    }
    ...
    public static void newIntance(Context context, String id, String status) {
        Intent intent = new Intent(context, CommodityEditActivity.class);
